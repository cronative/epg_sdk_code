...
class EPGCardScanner: UIViewController {
    ...
    private var viewGuide: CardTransparentView!
    ...
    private lazy var helperLabel: UILabel = {
        let view = UILabel()
        view.text = EPGCardScanner.appearance.helperText
    ...
    internal class func getScanner(resultsHandler: @escaping (_ number: String, _ date: String, _ name: String) -> Void) -> UIViewController {
        EPGCardScanner(resultsHandler: resultsHandler)
    }
...
//MARK: - Initialization & Add Views
private extension EPGCardScanner {
    ...
    func addVideoOutput() {
        ...
        videoOutput.setSampleBufferDelegate(self, queue: DispatchQueue(label: "com.etisalat.EPG.queue"))
    ...
    func addGuideView() {
        ...
        viewGuide = CardTransparentView(rectsArray: [CGRect(x: viewX, y: viewY, width: widht, height: height)])
...
extension EPGCardScanner: AVCaptureVideoDataOutputSampleBufferDelegate {
...
extension EPGCardScanner {
    internal class Appearance {
        internal var helperText = "Align the cards in the center."
    }
}
